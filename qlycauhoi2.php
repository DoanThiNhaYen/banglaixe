		...
		<div id="hienthi">
			...
			while($row=mysqli_fetch_array($kqpt))
			{
				...
				echo "<tr><td>Câu ".$row['id']."</td>
						<td><img src='/BT/365 câu/$cau'/height='400' width='740'></td>
						<td>Đáp Án:". $row['dapandung1']."</td>
						<td><a href='sua1.php?id=$id'><button type='button' class='btn btn-info'>Sửa</button></a> | <a href='xoa1.php?id=$id'><button type='button' class='btn btn-success'>Xóa</button></a></td></tr>";
			}
			echo "</table>";
					//4.tạo nút bấm 
					for($i=0;$i<=$sotrangdulieu;$i++)
					{
						echo "<a href='qlycauhoi2.php?trangchon=$i'> >>$i </a>";
					}
				?>
			</table>
		</div>
		<div id="link">
			<a href="themcauhoi1.php"><button type='button' class='btn btn-info'>Thêm câu hỏi</button></a>
		</div>
	</body>
</html>